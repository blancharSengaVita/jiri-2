<div class="create-jiri__add-contact add-contact">
    <div class="add-contact__container" x-cloak x-on:click.outside="{{ $modal }} = false">
        <p class="title--1"> Ajouter un projet </p>
        ...
        <div class="add-contact__container--list">
            <div class="add-contact__list list">
                ...
                <ul class="list__container">
                    @foreach($this->filteredProjects as $project)
                        <li class="list__item contact" wire:key="{{$project->id}}" wire:click="addToJiri({{ $project->id }})">
                            <input
                                class="display-none"
                                value="{{ $project->name }}"
                                type="checkbox"
                            ...
                            >
                            <label class="contact__card" for="project-{{ $project->id }}">{{ $project->name }}
                            </label>
                        </li>
                    @endforeach
                </ul>
            ...
            <div class="add-contact__list list">
                ...
                <ul>
                    @foreach($this->addedTojury as $project)
                        <li class="list__item contact--selected" wire:key="{{$project->id}}"
                            wire:click="deleteFromJiri({{ $project }})">
                            <input
                                class="display-none"
                                value="{{ $project->name }}"
                                type="checkbox"
                            ...
                            <label
                                ...
                                for="selectedProject-{{ $project->id }}">{{ $project->name }}
